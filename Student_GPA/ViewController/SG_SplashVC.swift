...
class SG_SplashVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        _ = Timer.scheduledTimer(timeInterval: 2.0, target: self, selector: #selector(SG_SplashVC.classmatelbl), userInfo: nil, repeats: false)
         _ = Timer.scheduledTimer(timeInterval: 4.0, target: self, selector: #selector(SG_SplashVC.schoollbl), userInfo: nil, repeats: false)
         _ = Timer.scheduledTimer(timeInterval: 6.0, target: self, selector: #selector(SG_SplashVC.insiderlbl), userInfo: nil, repeats: false)
         _ = Timer.scheduledTimer(timeInterval: 7.0, target: self, selector: #selector(SG_SplashVC.callFunction), userInfo: nil, repeats: false)
    }
    func classmatelbl(){
